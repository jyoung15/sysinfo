    ...
    once_cell::sync::Lazy::new(|| {
        unsafe {
            ...
            if libc::getrlimit(libc::RLIMIT_NOFILE, &mut limits) != 0 {
                // Most freebsd system now defaults to 1024.
...
fn boot_time() -> u64 {
    ...
    match sysctl::Ctl::new("kern.boottime").and_then(|c| c.value()) {
...
impl PidStatus {
    pub fn get_start_time(&self) -> Option<u64> {
        ...
        if let Some(second_str) = self.start_time.split(',').next() {
            u64::from_str(second_str).ok()
        } else {
            None
        }
    }
...
impl System {
    ...
    fn refresh_processors(&mut self, limit: Option<u32>) {
        if let Ok(f) = File::open("/proc/status") {
...
impl SystemExt for System {
    ...
    fn refresh_users_list(&mut self) {
        self.users = crate::freebsd::users::get_users_list();
    }
    ...
    fn get_load_average(&self) -> LoadAvg {
        if let Ok(Struct(loadavg)) = sysctl::Ctl::new("vm.loadavg").and_then(|c| c.value()) {
            let fscale = u8s_to_f64(&loadavg[16..20]);
            let one = u8s_to_f64(&loadavg[0..4]);
            let five = u8s_to_f64(&loadavg[4..8]);
            let fifteen = u8s_to_f64(&loadavg[8..12]);
            ...
            panic!("could not get load average");
        }
    }
...
fn parse_stat_file(data: &str) -> Result<PidStatus, ()> {
    /*
    from procfs(5):
    ...
    */
    let status: Vec<&str> = data.split(' ').collect();
    Ok(PidStatus {
        command_name: status[0].to_string(),
        pid: status[1].parse().unwrap(),
        parent_pid: status[2].parse().unwrap(),
        pgid: status[3].parse().unwrap(),
        sid: status[4].parse().unwrap(),
        tty: status[5].to_string(),
        flags: status[6].to_string(),
        start_time: status[7].to_string(),
        user_time: status[8].to_string(),
        system_time: status[9].to_string(),
        wchan: status[10].to_string(),
        credentials: status[11].to_string(),
        jail: status[12].to_string(),
    })
}
...
fn _get_process_data(
    ...
    let pid_status = parse_stat_file(&data)?;

    let parent_pid = if proc_list.pid != 0 {
        Some(proc_list.pid as Pid)
    } else {
        Some(pid_status.pid as Pid)
    };

    let mut p = Process::new(nb, parent_pid, pid_status.get_start_time().unwrap());

    p.stat_file = stat_file;
    // get_status(&mut p, pid_status[2]);
...
pub fn get_all_data<P: AsRef<Path>>(file_path: P, size: usize) -> io::Result<String> {
    ...
    let ret = get_all_data_from_file(&mut file, size);
    ret
}

fn get_uptime() -> u64 {
    match SystemTime::now().duration_since(SystemTime::UNIX_EPOCH) {
        Ok(n) => n.as_secs() - boot_time(),
        _ => panic!("cannot get uptime"),
    }
}
